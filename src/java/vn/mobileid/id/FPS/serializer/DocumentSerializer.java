...
public class DocumentSerializer implements JsonSerializable {
    ...
    public void serialize(JsonGenerator jg, SerializerProvider sp) throws IOException {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            try {
                jg.writeStringField("uploaded_on", Utils.convertToUTC_String(list.get(i).getCreatedAt()));
                jg.writeStringField("modified_at", Utils.convertToUTC_String(list.get(i).getModifiedAt()));
            } catch (Exception ex) {
            }
            jg.writeStringField("modified_by", list.get(i).getModifiedBy());
